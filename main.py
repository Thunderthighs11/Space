...
def get_time(img):
    try:
        ...
        exif_time = exif_dict["Exif"].get(piexif.ExifIFD.DateTimeOriginal, b"")
        return datetime.strptime(exif_time.decode(), "%Y:%m:%d %H:%M:%S")
    except Exception as e:
        print(f"Error in get_time for {img}: {e}")
        # Hvis der opstår fejl, returneres nuværende tid som fallback
        return datetime.now()
...
if __name__ == "__main__":
    ...
    with open("result.txt", "w") as file:
        file.write(f"Speed: {final_speed:.5g}")
